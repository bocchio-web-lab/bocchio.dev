...
<div class="container mx-auto space-y-8 py-8">
				...
				<Table.Root>
					...
					<Table.Body>
						{#each data.tenants as tenant}
							{@const userMembership = tenant.users?.find((u) => u.id === data.user?.id)}
							<Table.Row>
								...
								<Table.Cell class="text-right">
									<Button href={getPortalLink(tenant)} size="sm">Open Portal</Button>
								</Table.Cell>
							</Table.Row>
						{/each}
					</Table.Body>
	...
	<Card.Root>
		...
		<Card.Content>
								...
								{#if service.is_active}
									<Button
										...
										on:click={() => {
											...
											alert('Create tenant functionality coming soon');
										}}
									>
										Create Tenant
									</Button>
			...
			{:else}
				<p class="text-muted-foreground text-sm">No services available at the moment.</p>
			{/if}
		</Card.Content>
	</Card.Root>
</div>
