<script lang="ts">
	...
	const accessLevelOptions = [
		{ value: 'public', label: 'Public', description: 'Anyone can access your content' },
		{ value: 'private', label: 'Private', description: 'Only you and invited members can access' },
		{
	...
	const getAccessLevelLabel = (value: string) => {
		return accessLevelOptions.find((opt) => opt.value === value)?.label || value;
	};

	const getAccessLevelDescription = (value: string) => {
		return accessLevelOptions.find((opt) => opt.value === value)?.description || '';
	};
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			const result = await tenantApi.createTenant({
				...
				access_level: accessLevelValue as 'public' | 'private' | 'token_protected'
			});
		...
		<form onsubmit={handleSubmit}>
			...
			<div class="grid gap-4 py-4">
				...
				<div class="grid gap-2">
					<Label for="name">Tenant Name <span class="text-destructive">*</span></Label>
					<Input
						id="name"
						name="name"
						bind:value={name}
						...
						disabled={loading}
					/>
					<p class="text-xs text-muted-foreground">A descriptive name for your tenant</p>
				</div>
				...
				<div class="grid gap-2">
					...
					<p class="text-xs text-muted-foreground">
						Optional. Leave empty to auto-generate from name. Used in public URLs.
					</p>
				</div>

				<div class="grid gap-2">
					<Label for="access_level">Access Level <span class="text-destructive">*</span></Label>
					<Select.Root
						type="single"
						name="access_level"
